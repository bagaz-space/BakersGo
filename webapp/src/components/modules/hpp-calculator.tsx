...
import { useHppEntries, useSaveHpp, useDeleteHpp } from '@/hooks/useHpp';
        ...
        <>
          ...
          <SectionCard title="Zona Dapur — Biaya Operasional">
            <div className="grid grid-cols-2 gap-4">
              <CostInput label="Biaya Listrik (Rp)" value={listrik} onChange={setListrik} />
              <CostInput label="Biaya Gas (Rp)" value={gas} onChange={setGas} />
              <CostInput label="Upah Tenaga Kerja (Rp)" value={tenagaKerja} onChange={setTenagaKerja} />
              <CostInput label="Overhead Lainnya (Rp)" value={overhead} onChange={setOverhead} />
            </div>
          </SectionCard>
          ...
          <SectionCard title="Zona Final — Biaya Kemasan">
            <div className="grid grid-cols-2 gap-4">
              <CostInput label="Biaya Kotak/Box (Rp)" value={kotak} onChange={setKotak} />
              <CostInput label="Biaya Stiker (Rp)" value={stiker} onChange={setStiker} />
              <CostInput label="Kemasan Lainnya (Rp)" value={kemasanLain} onChange={setKemasanLain} />
            </div>
          </SectionCard>
              ...
              <div className="mt-4 pt-4 border-t border-[#A0813A]/20">
                {saved ? (
                  <div className="rounded-xl bg-green-500/10 border border-green-500/20 px-4 py-2.5 text-center">
                    <p className="text-sm font-medium text-green-600">HPP berhasil disimpan</p>
                  </div>
                ) : (
                  <button
                    onClick={handleSave}
                    disabled={saveHpp.isPending}
                  ...
                  >
                    {saveHpp.isPending ? 'Menyimpan...' : 'Simpan HPP ini'}
                  </button>
                )}
              </div>
            </div>
